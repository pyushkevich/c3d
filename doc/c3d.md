...
#### -alias, -antialias: Binary surface anti-aliasing

Syntax: `-antialias iso_surface_value`

Uses an anti-aliasing filter to estimate a surface location from a single label image. The
output is a level set image where the zero values are the estimated surface location. The
parameter *iso_surface_value* sets the surface value, for example 0.5 for a binary image
(max 1, min 0).

The process converges either at a fixed number of iterations, controlled by
**-iterations** or when the RMS error falls below a threshold, controlled by
**-antialias-rms**.

    c3d label.nii.gz -antialias 0.5 -o isosurface.nii.gz
