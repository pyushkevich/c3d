...
::operator() (bool test_header, bool test_voxels, double tol) 
{
  ...
  if(test_header)
    ...
    for(int i = 0; i < VDim; i++)
      {
      max_abs_diff = std::max(max_abs_diff, std::fabs(rx.GetSize()[i] - ry.GetSize()[i]));
      max_abs_diff = std::max(max_abs_diff, std::fabs(rx.GetIndex()[i] - ry.GetIndex()[i]));
      max_abs_diff = std::max(max_abs_diff, std::fabs(x->GetOrigin()[i] - y->GetOrigin()[i]));
      max_abs_diff = std::max(max_abs_diff, std::fabs(x->GetSpacing()[i] - y->GetSpacing()[i]));
      for(int j = 0; j < VDim; j++)
        {
        max_abs_diff = std::max(max_abs_diff, std::abs(x->GetDirection()[i][j] - y->GetDirection()[i][j]));
        }
      }
