...
=========================================================================*/
#ifndef __ImageCollectionConstIteratorWithIndex_h_
#define __ImageCollectionConstIteratorWithIndex_h_
...
public:
  ...
  ImageCollectionConstIteratorWithIndex(const SizeType radius, const RegionType &region)
    : m_TotalComponents(0), m_Radius(radius), m_Region(region), m_NeighborhoodSize(0) {}

  /** Add a scalar image to the collection */
  void AddScalarImage(TScalarImage *image)
    {
  ...
  void AddVectorImage(TVectorImage *image)
    ...
    m_NeighborhoodSize = iter.Size();
    std::vector<int> dest_offset;
    for(int i = 0; i < image->GetNumberOfComponentsPerPixel(); i++)
      {
      dest_offset.push_back(m_NeighborhoodSize * m_TotalComponents);
      m_TotalComponents++;
      }
  ...
  /** Add image via dynamic casting */
  void AddImage(itk::DataObject *image)
    {
    if (auto *scalar = dynamic_cast<TScalarImage *>(image)) AddScalarImage(scalar);
    else if (auto *vector = dynamic_cast<TVectorImage *>(image)) AddVectorImage(vector);
    else itkExceptionMacro("Wrong image type passed to ImageCollectionConstIteratorWithIndex");
    }
   ...
   */
  template <class TRandomAccessContainer>
  void GetNeighborhoodValues(TRandomAccessContainer &p)
    {
    for(auto &sid : m_ScalarImageData)
      {
      for(unsigned int i = 0; i < sid.iter.Size(); i++)
        p[sid.dest_offset[0]+i] = sid.iter.GetPixel(i);
      }
    for(auto &vid : m_VectorImageData)
      {
...
#endif // ImageCollectionConstIteratorWithIndex
